...
def fetch_tle(satellite_name: str) -> dict:
    ...
    url = f"{CELESTRAK_BASE_URL}?CATNR={sat_id}&FORMAT=TLE"
    ...
    try:
        ...
        if len(lines) < 3:
            return {"error": f"Invalid TLE data received. Response: {response.text[:200]}"}

        return {
            "satellite": lines[0].strip(),
            "line1":     lines[1].strip(),
            "line2":     lines[2].strip(),
            "fetched_at": datetime.now(timezone.utc).isoformat(),
            "source":    "CelesTrak"
...
def get_available_satellites() -> list:
    """Returns list of available satellite names."""
    return list(SATELLITE_IDS.keys())
